...
- [x] **Architecture Visuals**: Vector architecture asset in `docs/assets/trustdoc-ai-architecture.svg` embedded in `README.md`.
- [x] **Measured Benchmarks**: Benchmarked local pipeline and ONNX Judge latency (78.4 ms on CPU) saved in `latest_local_demo.json` and documented in `README.md`.
...
- Record a short screen-capture demo GIF or video showing `python -m trustdoc_ai demo` or the PySide UI.
- Apply GitHub repository metadata on github.com:
  ...
  - **Social Preview**: `docs/assets/trustdoc-ai-architecture.svg`
